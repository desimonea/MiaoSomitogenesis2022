...
paths.expname = 'ROI_H2B spiking_Mesp2_Segmentoid_Day4 start_6min interval_2022_04_03__16_42_48_Subset';
...
%% identify segment number and correct starting frame

%%
segment1_x = uint16([38.746,296.128]./xy_pxsize);
segment1_y = uint16([41.513,127.999]./xy_pxsize);
segment1_t0 = 35;
segment2_x = uint16([17.297,294.053]./xy_pxsize);
segment2_y = uint16([127.999,193.729]./xy_pxsize);
segment2_t0 = 75;
segment3_x = uint16([7.611,294.053]./xy_pxsize);
segment3_y = uint16([193.729,262.226]./xy_pxsize);
segment3_t0 = 125;
...
for i = [cell_struct_segments.cellId]
    ...
    % coverage
    if numel(cell_struct_i.frame) >= (frame_coverage * (max_frame-cell_struct_i.segment_t0))
...
for i = 1:max([cell_struct_trim.segment_num])
    % calculate mean segment intensity for each frame, by looping over time
    % and cells
...
%% test, this is just testing nested arrayfun has the same effect as nested looping
yy = [];
for y = 0:smallest_final
    xx = [];
    for x = cell_struct_trim([cell_struct_trim.segment_num]==1)
        xx = [xx x.intensity_ratio(x.frame-x.segment_t0==y)'];

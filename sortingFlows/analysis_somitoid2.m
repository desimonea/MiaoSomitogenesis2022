...
paths.expname = 'Somitoid2_ 72-84hr_6min interval_2021_09_18__15_24_26';
...
for i = [cell_struct.cellId]
    ...
    if numel(cell_struct_i.frame) >= (frame_coverage * max_frame)
        ...
        if sum(nonedge_filter) >= (0.8 * numel(cell_struct_i.frame))
            % forbid more than two divisions
...
%% surrounding intensity calculate ----- old
%% max proj
h5disp([paths.directory,paths.expname,'.h5'])
raw_image = h5read([paths.directory,paths.expname,'.h5'], '/data');
...
somitoid2_surrounding_last = raw_proj_filt(:,:,end);
save(['./cell_struct/','somitoid2_surrounding_last.mat'], "somitoid2_surrounding_last")
disp(['./cell_struct/','somitoid2_surrounding_last.mat' ' Saved']);
